"""
Defensive Performance — Correlations
...
Output plots: def_corr_1_heatmap.png, def_corr_2_pairwise_bars.png
"""
...
# Pearson Correlation Matrxi
corr = X.corr(method="pearson")
print("Pearson Correlation Matrix:")
print(corr.round(3).to_string())

# ── P-value matrix ────────────────────────────────────────────
p_matrix = pd.DataFrame(index=DEF_VARS, columns=DEF_VARS, dtype=float)
for c1 in DEF_VARS:
    for c2 in DEF_VARS:
        if c1 == c2:
...
# ── Pairwise summary ──────────────────────────────────────────
rows = []
for i, v1 in enumerate(DEF_VARS):
    for v2 in DEF_VARS[i+1:]:
...
ax.set_title("Defensive Variables — Correlation Heatmap", fontsize=13, pad=12)
plt.xticks(rotation=40, ha="right", fontsize=9)
plt.yticks(rotation=0, fontsize=9)
plt.tight_layout()
plt.savefig("def_corr_1_heatmap.png", dpi=150)
plt.close()
print("\nSaved: def_corr_1_heatmap.png")

# ── Plot 2: pairwise bar chart ────────────────────────────────
colors = summary["r"].apply(
    lambda r: "#3266ad" if abs(r) > 0.9 else "#1D9E75" if abs(r) > 0.5 else "#888780"
)
# Single-line labels: no multi-line text, so rows never overlap
labels = (summary["var1"] + "  vs  " + summary["var2"]).tolist()
n_bars = len(labels)

# 0.55 inch per row keeps labels readable at any count
...
ax.set_title("Defensive Variables — Pairwise Correlations", fontsize=13, pad=12)
...
plt.tight_layout()
plt.savefig("def_corr_2_pairwise_bars.png", dpi=150, bbox_inches="tight")
plt.close()
print("Saved: def_corr_2_pairwise_bars.png")